...
describe("GogoScraper", () => {
    ...
    test("loadEpisodes()", async () => {
        ...
        const result = await scraper.loadEpisodes("https://gogoanime.sk/category/one-piece");
    ...
    test("cache", async () => {
        ...
        const cachedResult = await scraper.loadEpisodes("https://gogoanime.sk/category/one-piece");
        ...
        expect(secondTimeEnd - secondTime).toBeLessThan((firstTimeEnd - firstTime) / threshold);
        GogoScraper.cache.clear();
    });
});
